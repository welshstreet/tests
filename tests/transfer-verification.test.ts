...
describe("=== TRANSFER CREDIT TESTS ===", () => {
    it("=== TRANSFER CREDIT CLAIM DONATE TRANSFER AGAIN ===", () => {
        // TEST SUMMARY
        ...
        // STEP 4a: deployer donates rewards
        ...
        // STEP 4a: deployer donates rewards DONATE_WELSH and DONATE_STREET
        
        if (disp) {
            console.log(`\nSTEP 4a: Deployer donates second round of rewards`);
        }
        
        // Execute second donation
        donateRewards(
            DONATE_WELSH,
            DONATE_STREET,
            deployer,
        ...
        // Total LP supply is still 30T (deployer 10T + wallet1 9T + wallet2 11T)
        const secondDonationIndexIncreaseA = Math.floor((DONATE_WELSH * PRECISION) / totalLpSupply);
        const secondDonationIndexIncreaseB = Math.floor((DONATE_STREET * PRECISION) / totalLpSupply);
        
        const globalIndexAfterSecondDonationA = globalIndexAfterTransferA + secondDonationIndexIncreaseA;
        const globalIndexAfterSecondDonationB = globalIndexAfterTransferB + secondDonationIndexIncreaseB;
        
        if (disp) {
            console.log(`\nGlobal indices after second donation:`);
            console.log(`  Second donation index increase: A=${secondDonationIndexIncreaseA}, B=${secondDonationIndexIncreaseB}`);
            console.log(`  New global indices: A=${globalIndexAfterSecondDonationA}, B=${globalIndexAfterSecondDonationB}`);
        }
        
        // Update deployer's unclaimed after second donation
        // Deployer has 10T LP at index=0
        const deployerAdditionalEarningsA = Math.floor((deployerInitialBalance * secondDonationIndexIncreaseA) / PRECISION);
        const deployerAdditionalEarningsB = Math.floor((deployerInitialBalance * secondDonationIndexIncreaseB) / PRECISION);
        
        const deployerUnclaimedAfterSecondDonationA = deployerUnclaimedAfterFirstTransferA + deployerAdditionalEarningsA;
        const deployerUnclaimedAfterSecondDonationB = deployerUnclaimedAfterFirstTransferB + deployerAdditionalEarningsB;
        
        if (disp) {
            console.log(`\nDeployer rewards after second donation:`);
            console.log(`  Additional earnings: A=${deployerAdditionalEarningsA}, B=${deployerAdditionalEarningsB}`);
            console.log(`  Total unclaimed: A=${deployerUnclaimedAfterSecondDonationA}, B=${deployerUnclaimedAfterSecondDonationB}`);
        }
        ...
        getRewardUserInfo(
            ...
            0,
            0,
            deployerUnclaimedAfterSecondDonationA,
            deployerUnclaimedAfterSecondDonationB,
            deployer,
            disp
        );
        
        // Update wallet1's unclaimed after second donation
        // wallet1 has 9T LP, debt=30B, preserve index from transfer
        // earned = (9T * (new_global - preserve_index)) / PRECISION
        const wallet1EarnedAfterSecondDonationA = Math.floor((wallet1ActualBalanceAfterTransfer * (globalIndexAfterSecondDonationA - actualPreserveIndexA)) / PRECISION);
        const wallet1EarnedAfterSecondDonationB = Math.floor((wallet1ActualBalanceAfterTransfer * (globalIndexAfterSecondDonationB - actualPreserveIndexB)) / PRECISION);
        
        const wallet1UnclaimedAfterSecondDonationA = wallet1EarnedAfterSecondDonationA > claimableA ? wallet1EarnedAfterSecondDonationA - claimableA : 0;
        const wallet1UnclaimedAfterSecondDonationB = wallet1EarnedAfterSecondDonationB > claimableB ? wallet1EarnedAfterSecondDonationB - claimableB : 0;
        
        if (disp) {
            console.log(`\nwallet1 rewards after second donation:`);
            console.log(`  Earned: A=${wallet1EarnedAfterSecondDonationA}, B=${wallet1EarnedAfterSecondDonationB}`);
            console.log(`  Debt: A=${claimableA}, B=${claimableB}`);
            console.log(`  Unclaimed: A=${wallet1UnclaimedAfterSecondDonationA}, B=${wallet1UnclaimedAfterSecondDonationB}`);
        }
        
        getRewardUserInfo(
            wallet1,
            wallet1ActualBalanceAfterTransfer,
            transferBlock,
            claimableA,
            claimableB,
            actualPreserveIndexA,
            actualPreserveIndexB,
            wallet1UnclaimedAfterSecondDonationA,
            wallet1UnclaimedAfterSecondDonationB,
            deployer,
            disp
        );
        ...
        // wallet2 received transfer AFTER first transfer, so preserve index was set at that time
        
        // Calculate wallet2's preserve index from FIRST transfer receipt
        ...
        // wallet2's earned at first transfer (with 10T at index=0)
        const wallet2EarnedAtFirstTransferA = Math.floor((wallet2OldBalanceBeforeReceive * (globalIndexAfterTransferA - 0)) / PRECISION);
        const wallet2EarnedAtFirstTransferB = Math.floor((wallet2OldBalanceBeforeReceive * (globalIndexAfterTransferB - 0)) / PRECISION);
        
        // wallet2's unclaimed at first transfer (debt=0, so unclaimed=earned)
        const wallet2UnclaimedAtFirstTransferA = wallet2EarnedAtFirstTransferA;
        const wallet2UnclaimedAtFirstTransferB = wallet2EarnedAtFirstTransferB;
        
        // wallet2's preserve index set during first transfer receipt
        const wallet2PreserveCalcA = Math.floor((wallet2UnclaimedAtFirstTransferA * PRECISION) / wallet2NewBalanceAfterReceive);
        const wallet2PreserveCalcB = Math.floor((wallet2UnclaimedAtFirstTransferB * PRECISION) / wallet2NewBalanceAfterReceive);
        
        const wallet2PreserveIndexTheorA = globalIndexAfterTransferA - wallet2PreserveCalcA;
        const wallet2PreserveIndexTheorB = globalIndexAfterTransferB - wallet2PreserveCalcB;
        
        // Apply precision adjustment
        const wallet2ActualPreserveIndexA = wallet2PreserveIndexTheorA - 1;
        const wallet2ActualPreserveIndexB = wallet2PreserveIndexTheorB - 1;
        
        // Now calculate wallet2's state AFTER second donation
        // wallet2's earned after second donation using preserve indices from first transfer
        const wallet2EarnedAfterSecondDonationA = Math.floor((wallet2NewBalanceAfterReceive * (globalIndexAfterSecondDonationA - wallet2ActualPreserveIndexA)) / PRECISION);
        const wallet2EarnedAfterSecondDonationB = Math.floor((wallet2NewBalanceAfterReceive * (globalIndexAfterSecondDonationB - wallet2ActualPreserveIndexB)) / PRECISION);
        
        // wallet2's unclaimed after second donation = earned - debt (debt=0 with our fix)
        const wallet2UnclaimedAfterSecondDonationA = wallet2EarnedAfterSecondDonationA;
        const wallet2UnclaimedAfterSecondDonationB = wallet2EarnedAfterSecondDonationB;
        
        if (disp) {
            console.log(`\nwallet2 state after second donation (before return transfer):`);
            console.log(`  Balance: ${wallet2NewBalanceAfterReceive}`);
            console.log(`  Preserve indices (from first transfer): A=${wallet2ActualPreserveIndexA}, B=${wallet2ActualPreserveIndexB}`);
            console.log(`  Earned: A=${wallet2EarnedAfterSecondDonationA}, B=${wallet2EarnedAfterSecondDonationB}`);
            console.log(`  Unclaimed: A=${wallet2UnclaimedAfterSecondDonationA}, B=${wallet2UnclaimedAfterSecondDonationB}`);
        }
        
        // wallet2's unclaimed NOW reflects earnings from BOTH donations
        const wallet2UnclaimedBeforeReturnA = wallet2UnclaimedAfterSecondDonationA;
        const wallet2UnclaimedBeforeReturnB = wallet2UnclaimedAfterSecondDonationB;
        ...
        const wallet2RetainedAfterReturnA = Math.floor(wallet2UnclaimedBeforeReturnA * wallet2ReturnTransferRatio);
        const wallet2RetainedAfterReturnB = Math.floor(wallet2UnclaimedBeforeReturnB * wallet2ReturnTransferRatio);
        
        const wallet2ForfeitedOnReturnA = wallet2UnclaimedBeforeReturnA - wallet2RetainedAfterReturnA;
        const wallet2ForfeitedOnReturnB = wallet2UnclaimedBeforeReturnB - wallet2RetainedAfterReturnB;
        ...
        const globalIndexAfterReturnA = globalIndexAfterSecondDonationA + globalIndexIncreaseFromReturnA;
        const globalIndexAfterReturnB = globalIndexAfterSecondDonationB + globalIndexIncreaseFromReturnB;
        ...
        const deployerShareOfWallet2ForfeitA = Math.floor((deployerInitialBalance * (globalIndexAfterReturnA - globalIndexAfterSecondDonationA)) / PRECISION);
        const deployerShareOfWallet2ForfeitB = Math.floor((deployerInitialBalance * (globalIndexAfterReturnB - globalIndexAfterSecondDonationB)) / PRECISION);
        
        const deployerUnclaimedAfterReturnTransferA = deployerUnclaimedAfterSecondDonationA + deployerShareOfWallet2ForfeitA;
        const deployerUnclaimedAfterReturnTransferB = deployerUnclaimedAfterSecondDonationB + deployerShareOfWallet2ForfeitB;
        ...
        // Rewards pool: Initial 100B + Second donation 100B = 200B total donated
        // Reduced by wallet1's claim of 30B
        const totalDonatedA = DONATE_WELSH * 2; // Two donations
        const totalDonatedB = DONATE_STREET * 2;
        const remainingRewardsA = totalDonatedA - claimableA;
        const remainingRewardsB = totalDonatedB - claimableB;
        ...
        if (disp) {
            ...
            console.log(`========================================`);
            console.log(`\nFirst donation: A=${DONATE_WELSH}, B=${DONATE_STREET}`);
            console.log(`Second donation: A=${DONATE_WELSH}, B=${DONATE_STREET}`);
            console.log(`Total donated: A=${totalDonatedA}, B=${totalDonatedB}`);
            ...
            console.log(`  A: ${totalAccountedA} (expected: ${totalDonatedA})`);
            console.log(`  B: ${totalAccountedB} (expected: ${totalDonatedB})`);
            
            const lostA = totalDonatedA - totalAccountedA;
            const lostB = totalDonatedB - totalAccountedB;
            
            if (lostA !== 0 || lostB !== 0) {
                console.log(`\n⚠️  REWARDS LOST:`);
                console.log(`  A: ${lostA} (${((lostA / totalDonatedA) * 100).toFixed(4)}%)`);
                console.log(`  B: ${lostB} (${((lostB / totalDonatedB) * 100).toFixed(4)}%)`);
            } else {
